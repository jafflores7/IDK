(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('zod'), require('express'), require('cors'), require('helmet'), require('express-rate-limit'), require('crypto'), require('perf_hooks')) :
    typeof define === 'function' && define.amd ? define(['exports', 'zod', 'express', 'cors', 'helmet', 'express-rate-limit', 'crypto', 'perf_hooks'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.core = {}, global.Zod, global.Express, global.cors, global.helmet, global.rateLimit, global.crypto, global.perf_hooks));
})(this, (function (exports, zod, express, cors, helmet, rateLimit, crypto, perf_hooks) { 'use strict';
    ...
    class ZodSchemaValidator {
        ...
        buildInputSchema(schema, _options) {
            ...
            const zodSchema = zod.z.object(zodFields);
            ...
            switch (field.type) {
                ...
                case 'object':
                    if (field.properties) {
                        ...
                        schema = zod.z.object(objectFields);
                        if (!field.additionalProperties) {
                            schema = schema.strict();
                        }
                    }
                    else {
                        schema = zod.z.record(zod.z.string(), zod.z.any());
                    }
                    break;
                case 'date':
                    schema = zod.z.coerce.date();
                    if (field.minDate) {
                ...
                case 'enum':
                    ...
                    schema = zod.z.enum(field.enum);
                    break;
                case 'json':
                    schema = zod.z.any().refine(val => {
                        try {
        ...
        buildConfigFieldSchema(field, fieldName, _options) {
            ...
            switch (field.type) {
                ...
                case 'apiKey':
                case 'secret':
                    schema = zod.z.string().min(1, `${fieldName} cannot be empty`);
                    break;
                case 'url':
                    schema = zod.z.string().url(`${fieldName} must be a valid URL`);
                ...
                case 'enum':
                    ...
                    schema = zod.z.enum(field.validation.enum);
                    break;
                case 'json':
                    schema = zod.z.any();
    ...
    class Tool {
            ...
            this.app.use(async (req, res, next) => {
                try {
                    ...
                    if (apiKey && this.validApiKeys.has(apiKey)) {
                        ...
                        req.headers['x-api-key-hash'] = crypto.createHash('sha256')
                            .update(apiKey)
                            .digest('hex')
                            .substring(0, 8);
                    }
        ...
        sendStandardError(res, error, requestId) {
            const errorResponse = {
                request_id: requestId || res.locals.requestId || crypto.randomUUID(),
                status: 'error',
                error: {
                    code: error.code,
                    message: error.message,
        ...
        async handleHealth(_req, res) {
            const healthCheckStart = perf_hooks.performance.now();
            try {
                ...
                if (this.definition.healthCheck) {
                    try {
                        const customHealthStart = perf_hooks.performance.now();
                        const customHealth = await this.definition.healthCheck();
                        const customHealthDuration = perf_hooks.performance.now() - customHealthStart;
                        checks.custom = {
                ...
                const healthCheckDuration = perf_hooks.performance.now() - healthCheckStart;
        ...
        async handleSchema(_req, res) {
            const schemaGenerationStart = perf_hooks.performance.now();
            try {
...
print(result)`,
                ...
                const schemaGenerationDuration = perf_hooks.performance.now() - schemaGenerationStart;
        ...
        async handleMetrics(_req, res) {
            const metricsGenerationStart = perf_hooks.performance.now();
            try {
                ...
                const enhancedMetrics = {
                    ...
                    configuration_metrics: {
                        rate_limiting: this.config.rateLimit
                            ...
                            : null,
                        authentication_enabled: this.config.security?.requireAuth || false,
                        monitoring_overhead_ms: Math.round(perf_hooks.performance.now() - metricsGenerationStart),
                    },
                    // Metadata
                    metrics_metadata: {
                        generated_at: new Date().toISOString(),
                        generation_time_ms: Math.round(perf_hooks.performance.now() - metricsGenerationStart),
                ...
                const metricsGenerationDuration = perf_hooks.performance.now() - metricsGenerationStart;
                this.sendStandardError(res, {
        ...
        async test(input, config) {
            try {
                ...
                const context = {
                    executionId: `test-${crypto.randomUUID()}`,
                    toolId: this.definition.metadata.name,
                    toolVersion: this.definition.metadata.version,
                    timestamp: new Date(),
                    environment: 'test',
