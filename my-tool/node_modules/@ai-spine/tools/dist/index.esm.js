import { Tool, ConfigurationError, ValidationError } from '@ai-spine/tools-core';
...
class ToolBuilder {
    ...
    validateBuilderState() {
        if (this._built) {
            throw new ConfigurationError('ToolBuilder has already been built and cannot be modified. Create a new builder instance.');
        }
    }
    /**
    ...
    build() {
        ...
        if (this._built) {
            throw new ConfigurationError('ToolBuilder has already been built. Create a new builder instance for a new tool.');
        }
        ...
        if (this._validationErrors.length > 0) {
            throw new ConfigurationError(`Tool configuration validation failed:\n${this._validationErrors.map(e => `  - ${e}`).join('\n')}`, this._validationErrors);
        }
...
function validateToolDefinition(options) {
    ...
    // Report errors
    if (errors.length > 0) {
        throw new ConfigurationError(`Tool definition validation failed:\n${errors.map(e => `  ❌ ${e}`).join('\n')}${warnings.length > 0
